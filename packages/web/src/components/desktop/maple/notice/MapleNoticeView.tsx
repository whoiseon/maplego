...
import { useGetGameNoticeView } from '@/lib/hooks/queries/game/useGetGameNoticeView';

function MapleNoticeView() {
  const params = useParams();
  const searchParams = useSearchParams();
  const postId = Number(params.postId);
  const target = searchParams.get('target') || 'all';

  const { data } = useGetGameNoticeView(postId, target as string);
...
export default MapleNoticeView;
